...
@alpha_log("alpha_limitup_notline")
def update_alpha_limitup_notline():
    # 常量
    _table_name = "alpha_limitup_notline"
    # 获取mongodb数据库连接
    client = MongoClass
    ...
    for code in stock_list:
        try:
            ...
            if _date == "0":
                _s_data = queryMarketData(code, fields=["code", "date", "high", "low", "close", "per_close"])
                if len(_s_data) == 0:
                    continue
                data_list = []
                for i in range(_s_data.shape[0]):
                    ...
                    if is_unyiziban_limit(_s_value):
                        _data_dict["value"] = 1
                    else:
                        _data_dict["value"] = 0
                ...
                _s_data = queryMarketData(code, start_date=related_date(_date, 1), fields=["code", "date", "high", "low", "close", "per_close"])
                if len(_s_data) == 0:
                    continue
                data_list = []
                for i in range(_s_data.shape[0]):
